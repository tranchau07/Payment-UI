...
apiClient.interceptors.request.use(
  (config) => {
    // Avoid putting auth header on authentication endpoints to keep them isolated
    const isAuthRequest = config.url.includes('/identity/login') || 
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      // If we are calling the new identity service, handle immediate logout without refresh token loop
      if (originalRequest.url && originalRequest.url.includes('/identity/')) {
        if (logoutCallback) {
          logoutCallback();
        }
        return Promise.reject(error);
