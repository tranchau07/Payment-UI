...
export default function ContractDetail() {
  ...
  const tabs = [{ id: 'overview', label: t('common.overview') }, { id: 'accounts', label: t('contract.accounts') }, { id: 'transactions', label: t('contract.transactions') }, { id: 'technical', label: t('common.technical') }]
    .filter((item) => item.id !== 'technical' || hasRole('SUPERVISOR') || hasRole('ADMIN'));
  const location = useLocation(); const contractId = location.pathname.split('/').filter(Boolean).at(-1);
  ...
  const load = () => { setLoading(true); setError(''); portfolioService.getContract(contractId).then(({ data: value }) => setData(value)).catch(() => setError(t('contract.loadError'))).finally(() => setLoading(false)); };
  useEffect(() => { let active = true; portfolioService.getContract(contractId).then(({ data: value }) => { if (active) setData(value); }).catch(() => { if (active) setError(t('contract.loadError')); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [contractId, t]);
  return <section className="transaction-detail-page"><Link to={data?.client?.id ? `/clients/${data.client.id}` : '/clients'} className="back-link">← {t('contract.customer')}</Link>
    <DataState loading={loading} error={error} onRetry={load} empty={!data}>{data && <><header className="transaction-detail-header card"><div><span>{t('contract.title')} #{data.id}</span><h2>{data.name || t('contract.unnamed')}</h2><MaskedIdentifier value={data.maskedContractNumber} /></div><div><MoneyAmount value={data.amountAvailable} currency={data.currency} /><small>{t('contract.available')}</small></div></header>
